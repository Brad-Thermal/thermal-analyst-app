...
def run_cobra_analysis(uploaded_file, cobra_data, selected_series, selected_ics, spec_df):
    try:
        ...
        # Chart Generation
        fig_chart, ax = plt.subplots(figsize=(max(10, len(df_table.index) * 0.8), 6))
        ...
        return {"table": df_table, "chart": fig_chart, "conclusion_data": conclusion_data}
...
def generate_formatted_table_image(df_table):
    ...
    if df_table.empty:
        ...
        ax.text(0.5, 0.5, "No data to display in table.", ha="center", va="center")
        ax.axis('off')
        return fig
